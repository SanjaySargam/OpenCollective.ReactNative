import { View, Text, StyleSheet, Button, TouchableOpacity, Image, TextInput, ScrollView, ActivityIndicator } from 'react-native'
...
const InfoScreen = () => {
  ...
  const data = [
    ...
    },
    {
      ...
      title: 'Company',
      placeholder: '',
      description: 'Start with @ to reference an organization (e.g., @airbnb)',
      value:``
    },
    {
      id: 4,
      ...
      <ScrollView style={styles.editContainer}>
        ...
        {data.map(({ title, placeholder, id, description, value }) => (
          <View style={styles.edit} key={id}>
            ...
            <TextInput placeholder={placeholder} style={[styles.input,focusedInput === id ? { borderColor: theme.textColor } : {},]} value={value}  onFocus={() => setFocusedInput(id)}
  onBlur={() => setFocusedInput(0)}/>
            <Text style={{ fontSize: 10, marginLeft: 8, marginRight: 8, color:theme.textColor }}>{description}</Text>
          </View>
        ))}
      </ScrollView>
      <TouchableOpacity style={styles.saveButton}>
        <Text style={{ textAlign: 'center', fontSize: 24, color: theme.backgroundColor, justifyContent: 'center', padding: 10, fontWeight: 'bold' }}>Save</Text>
      </TouchableOpacity>
    </View>
  )
}
